...
from dprox.contrib import mosaicing_torch
...
def build_model():
    ...
    def step_fn(gt):
        ...
        inp = mosaicing_torch(inp)
        inp = inp + torch.randn(*inp.shape, device=inp.device) * sigma
        y.value = inp
        PSF.value = psf

        out = solver.solve(x0=inp, rhos=rgb_collim_model.rhos, lams={reg_term: rgb_collim_model.sigmas.sqrt()}, max_iter=max_iter)
        return gt, inp, out
...
def test(
    ...
    result_dir="results/e2e_optics_dprox_joint",
):
...
def train(
    ...
    savedir="saved/e2e_optics_dprox_joint/",
    epochs=10,
    bs=2,
    lr=1e-4,
    resume=None,
    ...
    optimizer = torch.optim.AdamW(rgb_collim_model.parameters(), lr=1e-4, weight_decay=1e-3)

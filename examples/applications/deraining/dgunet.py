...
class ConvBlock(nn.Module):
    def __init__(self, in_channel, out_channel, strides=1, bias=False):
        super(ConvBlock, self).__init__()
        self.strides = strides
        self.in_channel = in_channel
        self.out_channel = out_channel
        self.block = nn.Sequential(
            nn.Conv2d(in_channel, out_channel, kernel_size=3, stride=1, bias=bias, padding=1),
            nn.LeakyReLU(inplace=True),
            nn.Conv2d(out_channel, out_channel, kernel_size=3, stride=strides, bias=bias, padding=1),
            nn.LeakyReLU(inplace=True),
        )
        self.conv11 = nn.Conv2d(in_channel, out_channel, kernel_size=1, bias=bias, stride=strides, padding=0)
...
def conv(in_channels, out_channels, kernel_size, bias=False, stride=1):
    ...
    return ConvBlock(in_channels, out_channels, stride, bias)


def conv_down(in_chn, out_chn, bias=False):
    # layer = nn.Conv2d(in_chn, out_chn, kernel_size=4, stride=2, padding=1, bias=bias)
    layer = ConvBlock(in_chn, out_chn, 2, bias)
    return layer
...
class Basic_block(nn.Module):
    ...
    def prox(self, v, feat1, res1, x2_samfeats):
        x2 = self.shallow_feat2(v)
        ...
        x3_samfeats, x = self.sam23(res2[-1], v)
        return x, (feat2, res2, x3_samfeats)


##########################################################################
# DGUNet_plus
def clone(x, num):
    return [copy.deepcopy(x) for _ in range(num)]

class Denoiser(nn.Module):
    def __init__(
        self, in_c=3, out_c=3, n_feat=40, scale_unetfeats=20, scale_orsnetfeats=16, num_cab=8, kernel_size=3, reduction=4, bias=False,
        depth=5,
    ):
        super().__init__()
        act = nn.PReLU()
        self.basic = nn.ModuleList(
            clone(Basic_block(in_c, out_c, n_feat, scale_unetfeats, scale_orsnetfeats, num_cab, kernel_size, reduction, bias), 5)
        )
    ...
    def forward(self, v, depth):
        if depth == 0:
            x1 = self.shallow_feat1(v)
            feat1, feat_fin1 = self.stage1_encoder(x1)
            res1 = self.stage1_decoder(feat_fin1, feat1)
            x2_samfeats, x = self.sam12(res1[-1], v)
            self.x2_samfeats = x2_samfeats
            self.context = feat1, res1, x2_samfeats
            return x
        elif depth == self.depth + 1:
            x7 = self.shallow_feat7(v)
            x7_cat = self.merge67(x7, self.x2_samfeats)
            x = self.tail(x7_cat)
            return x
        else:
            x, (feat1, res1,  x2_samfeats) = self.basic[depth-1].prox(v, *self.context)
            self.x2_samfeats = x2_samfeats
            self.context = feat1, res1, x2_samfeats
            return x
...
class LearnableDegOp(nn.Module):
    ...
    def forward(self, x, step=None):
        ...
        return getattr(self, f'phi_{step}')(x)

    def adjoint(self, x, step=None):
        if step is None: step = self.step
        return getattr(self, f'phit_{step}')(x)

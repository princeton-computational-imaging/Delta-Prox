...
from DGUNet_plus2 import DGUNet
import numpy as np
import os
import argparse
...
parser.add_argument('--result_dir', default='./results/DGUNet_restormer2_plus_tune_epoch32-2', type=str, help='Directory for results')
...
# model_restoration.load_state_dict(torch.load('restormer.pth')['params'])
# from DGUNet_conv import DGUNet
model_restoration = DGUNet()

utils.load_checkpoint(model_restoration, args.weights)
# model_restoration.load_state_dict(utils.remove_module('DGUNet_plus.pth'), strict=False)
...
for dataset in datasets:
    ...
    with torch.no_grad():
        for ii, data_test in enumerate(tqdm(test_loader), 0):
            ...
            # restored = test_x8(model_restoration,input_)
            saved = torch.cat(restored, dim=0)
            saved = torch.clamp(saved, 0, 1)
            torch.save({'output': saved}, 'output.pth')

...
def generate_lp_dense_problem(n, m1, m2, dtype=torch.float64, seed=0):
...
def generate_lp_sparse_problem(n, m1, m2, dtype=np.float64, seed=0):
    n: int   # dimension of x
    m1: int  # number of equality constraints
    m2: int  # number of inequality constraints
    ...
    c = np.random.rand(n, 1) + 0.5  # create nonnegative price vector with mean 1
    x0 = np.abs(np.random.randn(n, 1))
    A = sparse.rand(m1, n, density=1e-2)  # create random, nonnegative matrix A    
    G = sparse.rand(m2, n, density=1e-2)  # create random, nonnegative matrix A
    b = A @ x0
    g = G @ x0 * 1.2
    lpproblem = LPProblem(c, G, g, A, b, norm_ord=float('inf'), sparse=True, device=device)
    return lpproblem


def test_lp_problem(n, m1, m2, seed=0, max_iters=10000, abstol=1e-4, reltol=1e-5, dtype=torch.float64, sparse=False):
    if sparse:
        lpproblem = generate_lp_sparse_problem(n, m1, m2, seed)
    else:
        lpproblem = generate_lp_dense_problem(n, m1, m2, dtype, seed)
    c, A_ub, b_ub, A_eq, b_eq, x_lb, x_ub = lpproblem.unpack()
    ...
    start = time.time()
    res = sop.linprog(c=c, A_ub=A_ub, b_ub=b_ub, A_eq=A_eq, b_eq=b_eq, bounds=(0, None), method='highs-ipm',
                      options={'maxiter': 10000, 'disp': True, 'presolve': False, 'autoscale': True})
    print(res.fun, res.success, res.status)
    # feasibility check
    x = res.x
    obj = c.flatten() @ x.flatten()
    print(f'cTx: {(obj):.4f}')
    print(f'x >= 0 err: {x.min():.2e}')
    print(f'Ax = b err: {np.abs(b_eq - A_eq @ x).max():.2e}')
    print(f'Ax <= b err: {min(np.min(b_ub - A_ub @ x), 0):.2e}')
    end = time.time()
    print(f"Time Elapse (Highs): {end-start:.4f}s\n")
    
    start = time.time()
    with torch.no_grad():
        x, history, res = lpadmm.solve(lpproblem, rho=1e-3, alpha=1, residual_balance=True, direct=False, polish=False)
    print(res[0])
...
def test_lp_general_sparse(seed=0, max_iters=int(2e6), abstol=1e-6, reltol=1e-12):
    ...
    c, A_ub, A_eq, b_ub, b_eq, x_lb, x_ub = load_simple_cep_model() 
    # A_ub, b_ub, A_eq, b_eq, c, x_lb, x_ub = load_lp_componentes_from_mat('./IESP_P1.mat')
    ...
    lpadmm = LPSolverADMM(rho=1e-1, problem_scale=None, abstol=abstol, reltol=reltol, max_iters=max_iters, dtype=dtype).to(device)
    ...
    # start = time.time()
    # res = sop.linprog(c=c, A_ub=A_ub, b_ub=b_ub, A_eq=A_eq, b_eq=b_eq, bounds=list(zip(x_lb, x_ub)), method='highs-ipm',
    #                   options={'maxiter': 10000, 'disp': True, 'presolve': False})
    ...
    logging.info(f"Time Elapse (DProx inference stage): {end-start:.4f}s\n")
    
    
if __name__ == '__main__':
    # test_lp_problem(n=2000, m1=1000, m2=500, seed=0, max_iters=int(1e5), abstol=1e-4, reltol=1e-10, sparse=True)

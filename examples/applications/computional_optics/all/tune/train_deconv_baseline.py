...
from common import (Dataset, build_doe_model, circular, device,
                    load_sample_img, plot, sanity_check, subplot,build_baseline_profile)
...
def main(args):
    ...
    solver.rhos = nn.parameter.Parameter(rhos, requires_grad=trainable_params)
    solver.sigmas = nn.parameter.Parameter(sigmas, requires_grad=trainable_params)
    ...
    def step_fn(gt):
        ...
        out = solver.solve(x0=inp,
                           rhos=solver.rhos,
                           lams={reg_term: solver.sigmas},
                           max_iter=max_iter)
        return gt, inp, out

    # ----------------- Sanity Check ------------------------ #
    gt, ob = sanity_check(rgb_collim_model.get_psf(fresnel_phase_c))
    gt, inp, out = step_fn(gt)
    logger.info(tl.metrics.psnr(out, gt))  # 38.55
    imshow(gt, ob, out)

    logger.info('Model size (M) {}'.format(tlnn.benchmark.model_size(rgb_collim_model.height_map)))
    ...
    if args.resume:
        ...
        solver.load_state_dict(ckpt['model'])
...
def parse_args():
    ...
    parser.add_argument('--savedir', type=str, default='saved/train_deconv_baseline/')

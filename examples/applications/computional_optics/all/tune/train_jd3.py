...
def main(args):
    ...
    # rgb_collim_model.height_map.requires_grad_(False)
    
    # ckpt = torch.load( 'saved/train_deconv_trainable_params/best.pth')
    ...
    while epoch < args.epochs:
        ...
        for i, batch in enumerate(loader):
            ...
            if NUM_ACCUMULATES > 0:
                ...
                loss.backward()
                norm = nn.utils.clip_grad_norm_(rgb_collim_model.parameters(), 1)
                optimizer.step()
                optimizer.zero_grad()
            ...
            if (gstep + 1) % 50 == 0:
                torchvision.utils.save_image(inp[:1], imgdir / f'inp_{gstep}.png')
                torchvision.utils.save_image(pred[:1], imgdir / f'pred_{gstep}.png')
                torchvision.utils.save_image(gt[:1], imgdir / f'gt_{gstep}.png')

                psf = rgb_collim_model.get_psf()
                subplot(psf[:1], imgdir / f'psf_{gstep}.png')

                height_map = rgb_collim_model.height_map.height_map_sqrt.detach()**2
                plot(height_map[:1], imgdir / f'height_map_{gstep}.png')

                phase = torch.angle(rgb_collim_model.height_map.get_phase_profile())
                subplot(phase[:1], imgdir / f'phase_{gstep}.png')
            gstep += 1
...
def parse_args():
    ...
    parser.add_argument('--savedir', type=str, default='saved/train_jd3/')
